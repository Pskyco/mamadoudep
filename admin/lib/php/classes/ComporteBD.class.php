...
class ComporteBD extends Comporte {
    ...
    *   paramètres : fk_facture, fk_produit & quantité
    *   permet d'effectuer un insert dans la base de données
    *   retour : > 0 si c'est ok, -1 si erreur
    */
    public function ajoutComporte($facture,$produit,$quantite) {
        $retour = array();
        try {
            $query = "select comporte_ajout(:facture,:produit,:quantite) as retour";
